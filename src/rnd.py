...
def eval(energy, buffer, gfn_model, final=False):
    ...
    init_state = torch.zeros(args.eval_size, energy.data_ndim).to(args.device)
    teacher_samples = buffer.sample_pos(args.eval_size).to(args.device)
    gt_samples = energy.sample(args.eval_size).to(args.device)
    
    samples, metrics[f'{eval_dir}/log_Z_IS'], metrics[f'{eval_dir}/ELBO'], metrics[f'{eval_dir}/log_Z_learned'] = log_partition_function(init_state, gfn_model, energy.log_reward)
    ...
    if args.sampling == 'sleep_phase':
        ...
        if args.local_search:
            if it % args.ls_cycle < 2:
                ...
                local_search_samples, log_r = langevin_dynamics(samples, energy.log_reward, args.device, args)
...
def train(name, energy, buffer, buffer_ls, epoch_offset, log_Z_est=None, pre_model=None):
    ...
    gfn_model.eval()
    with torch.no_grad():
        metrics.update(eval(energy, buffer, gfn_model, True))
    if 'tb-avg' in args.mode_fwd or 'tb-avg' in args.mode_bwd:
...
if __name__ == '__main__':    
    ...
    rnd_model = RNDModel(args, energy.data_ndim, args.energy).to(args.device)
    rnd_model.load_state_dict(torch.load(f'result/2025-04-22_18:15:12/rnd_10000.pt'))
    ...
    gfn_model.load_state_dict(torch.load(f'result/2025-04-22_18:15:12/policy_10000.pt'), strict=False)
    ...
    samples = samples[-args.buffer_size//2:]
    rewards = rewards[-args.buffer_size//2:]
    
    buffer.add(samples.detach().cpu(), rewards.detach().cpu())
    np.save(f'{name}/rnd_positions.npy', samples.cpu().numpy())
    np.save(f'{name}/rnd_rewards.npy', rewards.cpu().numpy())
