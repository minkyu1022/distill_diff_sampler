...
@torch.no_grad()
def log_partition_function(log_pfs, log_pbs, log_fs, log_rs):
    log_weight = log_rs + log_pbs.sum(-1) - log_pfs.sum(-1)

    log_Z_IS = logmeanexp(log_weight)
    log_Z_lb = log_weight.mean()
    log_Z_learned = log_fs[:, 0].mean()

    return log_Z_IS, log_Z_lb, log_Z_learned
...
@torch.no_grad()
def EUBO(log_pfs, log_pbs, log_rs):
    log_weight = log_rs + log_pbs.sum(-1) - log_pfs.sum(-1)
