...
from tbg.tbg.gcl import E_GCL_vel, E_GCL, GCL
from tbg.tbg.utils import remove_mean, remove_mean_with_mask
...
class EGNN_dynamics_AD2_cat(nn.Module):
    def __init__(self, n_particles, n_dimension, h_initial, hidden_nf=64, device='cpu',
        ...
        if mode in ['egnn_dynamics', 'egnn_flow']:
    ...
    def forward(self, t, xs):
        ...
        if self.mode == 'egnn_dynamics':
            ...
            self.counter += 1
            return vel.view(n_batch,  self._n_particles* self._n_dimension)
        elif self.mode == 'egnn_flow':
            edge_attr = torch.sum((x[edges[0]] - x[edges[1]])**2, dim=1, keepdim=True)
            h_final, _ = self.egnn(h, x, edges, edge_attr=edge_attr)
            h_final = h_final.reshape(n_batch, self._n_particles, -1).mean(1)
            flow = self.final_layer(h_final)
            self.counter += 1
            return flow
        else:
            raise NotImplemented()
