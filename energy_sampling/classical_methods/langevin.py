...
def langevin_dynamics(x, log_reward, device, args):
    ...
    accepted_logr = []
    acceptance_rate_lst = []
    log_r_original = log_reward(x, count=True)
    acceptance_count = 0
    acceptance_rate = 0
    total_proposals = 0
    ...
    for i in range(args.max_iter_ls):
        ...
        r_grad_original = torch.autograd.grad(log_reward(x, count=True).sum(), x)[0]
        if args.ld_schedule:
        ...
        log_r_new = log_reward(new_x, count=True)
        r_grad_new = torch.autograd.grad(log_reward(new_x, count=True).sum(), new_x)[0]
