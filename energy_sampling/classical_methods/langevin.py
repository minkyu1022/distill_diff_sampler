...
def langevin_dynamics(x, log_reward, device, args, kde_estimator=None):
    accepted_samples = []
    accepted_logr = []
    acceptance_rate_lst = []
    ...
    for i in range(args.max_iter_ls):
        ...
        if kde_estimator is not None:
            kde_score = torch.autograd.grad(kde_estimator.log_prob(x).sum(), x)[0]
            drift = r_grad_original.detach() - kde_score.detach()
        else:
