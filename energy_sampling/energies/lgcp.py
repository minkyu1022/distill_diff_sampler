...
_LGCP_DIM = 1600
_CSV_PATH = "energy_sampling/energies/data/pines.csv"
...
class Cox:
    def __init__(self, file_name, num_bins_per_dim, use_whitened, device):
        ...
        # torch
        self._bin_vals = (
            torch.from_numpy(get_bin_vals(num_bins_per_dim)).to(self.device).float()
        )
        ...
        self._flat_bin_counts = (
            torch.from_numpy(self._flat_bin_counts).to(self.device).float()
        )
...
class CoxDist(BaseSet):
    ...
    _ground_truth_logZ = 512.6
    can_sample = False

    def __init__(self, device, dim=1600):
        super().__init__(device, _LGCP_DIM)
    ...
    def viz_pdf(self, fsave="cox-density.png"):
        raise NotImplementedError
